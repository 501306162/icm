function displacement_ind=displacement(ind,image,N,range)
% return the neighbors'index of 'ind ' pixel 
switch (N)
    case 8
       displacement_ind= displacement2d(ind,image,range);
    case 26
       displacement_ind= displacement3d(ind,image,range);   
    otherwise
...
function displacement_ind=displacement3d(ind,image,range)
% return the neighbors'index of 'ind ' pixel

[r,c,s]=ind2sub(size(image),ind);
subfirst=[range,range,range];
subend=size(image);
...
% the neighbors include the 'ind' pixel
displacement_ind=sub2ind(size(image),row(:),col(:),slice(:));
